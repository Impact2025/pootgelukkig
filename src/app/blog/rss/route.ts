...
export async function GET() {
  let items: { titel: string; slug: string; excerpt: string | null; gepubliceerdOp: Date | null }[] = []

  try {
    items = await db
      .select({ titel: blogPosts.titel, slug: blogPosts.slug, excerpt: blogPosts.excerpt, gepubliceerdOp: blogPosts.gepubliceerdOp })
      .from(blogPosts)
